...
-   At the bottom of the **Access** tab, select the box that says
    **Available for Dashboard?**. Users with permissions to see that
    information will be able to add that dashlet. (See the [/initial-set-up/permissions-and-access-control.md](section on permissions) for more information).
...
(Refer to the [/reporting/what-is-civireport.md](Reporting section) for more details on working with reports.)
